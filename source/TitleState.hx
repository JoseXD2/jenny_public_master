...
class TitleState extends MusicBeatState
{
	...
	public var doorDown:FlxSprite;
	public var doorUP:FlxSprite;
	...
	function startIntro()
	{
		...
		if (!FileSystem.exists(path)){
			path = "assets/images/JennyMenu.png";
		//trace(path, FileSystem.exists(path));
		}
		gfDance.frames = FlxAtlasFrames.fromSparrow(BitmapData.fromFile(path),File.getContent(StringTools.replace(path,".png",".xml")));
		#else
		
		gfDance.frames = Paths.getSparrowAtlas('JennyMenu');
		#end
		gfDance.animation.addByPrefix('idle', "JennyMenuBop", 24);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				animIntroDoors();
				FlxG.sound.play(Paths.sound('TransitionIN'), 0.9);
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
...

			{
				...
				{
					if(gfDance.angle == -2) 
						FlxTween.angle(gfDance, gfDance.angle, 2, 2, {ease: FlxEase.quartInOut});
					if (gfDance.angle == 2) 
						FlxTween.angle(gfDance, gfDance.angle, -2, 2, {ease: FlxEase.quartInOut});
				}, 0);
			}
	...
	function animIntroDoors():Void{
		FlxTween.tween(doorUP,  { y: -82}, 0.4, {ease: FlxEase.sineOut});
		FlxTween.tween(doorDown, { y: 260}, 0.4, {ease: FlxEase.sineOut});
		
		new FlxTimer().start(0.1, function(tmr:FlxTimer)
			{
				FlxTween.tween(gear, { alpha: 1}, 0.2, {ease: FlxEase.sineOut});
			});
		new FlxTimer().start(0.2, function(tmr:FlxTimer)
			{
				FlxTween.tween(gear, {angle: 1080}, 1, {ease: FlxEase.sineOut});
			});
	}
}
